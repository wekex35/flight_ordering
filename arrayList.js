...
for (let index = 0; index < data.length; index++) {
    const d = data[index];
    var lProducts = [];
    for (let jindex = 0; jindex < d['menu-items'].length; jindex++) {
        const element = d['menu-items'][jindex];
        for (let eindex = 0; eindex < element['sub-items'].length; eindex++) {
            const m = element['sub-items'][eindex];
            m.stock = Math.floor(Math.random() * (10 - 0 + 1)) + 0;
...
fs.writeFile('productJson.csv', JSON.stringify(products,null,"  "), function (err) {
    if (err) throw err;
